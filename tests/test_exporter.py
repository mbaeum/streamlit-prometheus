...
def test__port_is_open(mock_socket_cls):
    ...
    mock_sock.connect_ex.assert_called_once_with((_DEFAULT_HOST, 8080))
...
def test_start_exporter_starts_server_and_logs(mock_start_http_server):
    ...
    mock_start_http_server.assert_called_once_with(port=port, addr=_DEFAULT_HOST)
